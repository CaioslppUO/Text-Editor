	...
	public void decideEditorEnabled(Boolean isFileAlreadyOpen) {
		if(this.currentFile != null) {
			this.editorPane.setEnabled(true);
                        if(!isFileAlreadyOpen) createNewFileOpenPanel(this.currentFile.getName(), this.currentFile.getAbsolutePath());
		}else {
            ...
            if(this.lastClickedFilePath == null){
                ...
                for(Component c:this.addedFilesPanel.get(this.lastClickedFilePath).getComponents()){
                    if(c instanceof JLabel){
                        ((JLabel)c).setForeground(this.MenuForeGroundColor);
                    }
                }
                this.lastClickedFilePath = filePath;
            }
	...
	private void saveFile(Boolean showSaveMessage) {
		if(this.currentFile != null) {
			try {
				...
				print_line.close();
                                if(showSaveMessage) JOptionPane.showMessageDialog(null, "File Saved");
			} catch (IOException e) {
            ...
            if(defineCreateOpenFileFrame() == JFileChooser.APPROVE_OPTION) { //Abre o arquivo
                ...
                try {
                    ...
                    try {
                        ...
                        if(this.addedFilesPanel.get(this.currentFile.getAbsolutePath()) != null) this.decideEditorEnabled(true);
